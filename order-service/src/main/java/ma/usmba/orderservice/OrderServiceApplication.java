package ma.usmba.orderservice;

import ma.usmba.orderservice.entities.Order;
import ma.usmba.orderservice.entities.ProductItem;
...
public class OrderServiceApplication {
    ...
    CommandLineRunner start(OrderRepository orderRepository,
        ...
        return args -> {
            List<Customer> customers=customerRestClientService.pageCustomers().getContent().stream().toList();
            List<Product> products=inventoryRestClientService.pageProducts().getContent().stream().toList();
            Long customerId=1L;
